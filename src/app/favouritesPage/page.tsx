...
const FavouritesPage = () => {
  ...
  return (
    <main className={`min-h-screen bg-gradient-to-br ${theme === "dark" ? "from-gray-900 to-gray-800" : "from-pink-50 to-pink-100"} py-8 sm:py-12 px-2 sm:px-4`}>
      <div className="">
        <div className="flex items-center gap-2 sm:gap-3 mb-6 sm:mb-8">
          <FaHeart className="text-pink-500 text-2xl sm:text-3xl" />
          <h1 className="text-2xl sm:text-3xl font-bold text-pink-700">My Favorites</h1>
        </div>
        {favorites.length === 0 ? (
          ...
          <div className={`grid max-[900px]:grid-cols-1 grid-cols-6 xs:grid-cols-2 gap-4 sm:gap-8`}>
            {favorites.map(item => (
              <div key={item.id} className={`${theme === "dark" ? "bg-gray-800 border-1 text-white" : "bg-white text-gray-900"}  rounded-xl shadow-lg p-4 sm:p-6 flex flex-col items-center relative w-full`}>
